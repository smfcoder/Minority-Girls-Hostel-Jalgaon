...
<?php
    ...
    {
        $slug = $_POST['slug'];
        header('location:/admin/delfile.php?slug='.$slug);
    }
?>
<!DOCTYPE html>
<html lang="en">
...
<?php
    ...
    if(isset($_SESSION['msg'])) {
        if($_SESSION['msg']=='success')   {
            ...
            echo '<script>alert("Sorry :( Failed Deletion)")</script>';
            unset($_SESSION['msg']);
        }
    }
?> 
  ...
  ?>
    ...
    <form method="POST" action="delselectfile.php">


        
        <div class="form-group row">
            <label for="" class="col-sm-2 col-form-label">Select to Delete File</label>
            <div class="col-sm-10">
               <select class="custom-select my-1 mr-sm-2" id="inlineFormCustomSelectPref" onchange="uniqueUser()" name="slug" required>
                ...
                <?php
                        ...
                        {  
                                ...
                                {
                                    $s=$row['slug'];
                                    $parent="SELECT * FROM file where slug='$s'";  
                                    $eparent = mysqli_query($sql, $parent);  
                                    $noc = mysqli_num_rows($eparent);
                                    
                                    if($row['type']=="file" && $noc!=0)
                                    {
                        ...
                        {  
                                while($rows = mysqli_fetch_array($ec))
                                {
                                    ...
                                    $s=$rows['slug'];
                                    $child="SELECT * FROM file where slug='$s'";  
                                    $echild = mysqli_query($sql, $child);  
                                    $noc = mysqli_num_rows($echild);
                                    
                                    if($rows['type']=="file" && $noc!=0)
                                    {
